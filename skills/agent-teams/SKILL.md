...
**TDD 절차 (반드시 준수):**
1. PRD 수용 기준 → 테스트 케이스로 변환 (테스트 먼저 작성)
2. 테스트 실행 → 실패 확인 (Red)
...
**TDD 절차 (반드시 준수):**
1. API 명세 → 테스트 케이스로 변환 (테스트 먼저 작성)
2. 테스트 실행 → 실패 확인 (Red)
3. 최소한의 코드로 테스트 통과 (Green)
4. 리팩토링 (Refactor)
5. 각 엔드포인트/서비스마다 이 사이클 반복
...
L1 Unit: (테스트 프레임워크에 따라 실행)
  **TDD 검증: 각 구현 파일에 대응하는 테스트 파일이 존재하는지 확인.**
  **테스트 파일이 없는 구현 파일 발견 시 → FAIL 처리 후 해당 개발자에게 테스트 작성 요청.**
...
이슈 발견 시 직접 수정.
버그 발견 → .opensmith/bugs.json에 즉시 기록.
개선사항/PRD 누락 발견 → TaskCreate로 TODO 즉시 등록.
완료 후 TaskUpdate"
)
```
...
```
작업 중 이상 발견:
  ├─ 버그/결함 (기존 코드 문제, 런타임 에러, 보안 취약점 등)
  │   → PRD Section 9 Bugs 테이블에 행 추가:
  │   | BUG-NNN | [설명] | [critical/high/medium/low] | open | [역할명] | [날짜] | [파일] |
  │
  └─ TODO (개선사항, 리팩토링 필요, 미비한 점, PRD 누락 등)
      → PRD Section 10 TODOs 테이블에 행 추가:
      | TODO-NNN | [설명] | [P0/P1/P2] | open | [역할명] | [날짜] |
```

이렇게 하면:
- 기능별로 Bug/TODO가 PRD에 모여있어 한눈에 파악 가능
- QA(step7)에서 PRD의 Bug/TODO 목록을 확인하여 전부 해결됐는지 검증
- 세션이 끊겨도 디스크에 영구 저장됨
